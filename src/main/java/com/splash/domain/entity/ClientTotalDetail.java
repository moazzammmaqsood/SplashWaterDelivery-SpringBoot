...
@NamedNativeQuery(name="OrderEntity.getClientTotalDetail",query=" SELECT clientid ,sum(bottlesdelivered) as totalbottles ,sum(bottlesrecieved) as totalrecieved , sum(payment) as totalpayment , max(date) date ,sum(bottlesdelivered*rate) as bill FROM orders where clientid = ?1 and status != 'D' group by clientid ",resultClass =ClientTotalDetail.class)
